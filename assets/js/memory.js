	...
	showItem: function (item) {
		...
		item.addClass('visible').find('img').fadeIn(200, function () {
			...
			if (v.length == 2) {
				...
				let encrypt_1 = v.filter(':eq(0)').find('img').data('code');
				let encrypt_2 = v.filter(':eq(1)').find('img').data('code');

				console.log(encrypt_1);
				console.log(encrypt_2);

				$.ajax({
					type: "POST",
					url: 'http://localhost/Memory-Game-master/Welcome/checkCard',
					data: {
						encrypt_1:encrypt_1,
						encrypt_2:encrypt_2
					},
